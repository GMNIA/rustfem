...
fn edge2d_basic_behaviour() {
    let edge = Edge2d::new(Vector2d::new(0.0, 0.0), Vector2d::new(3.0, 0.0));
    assert_almost_eq!(edge.length(), 3.0);
    assert!(edge.contains(&Vector2d::new(1.5, 0.0)));
...
fn edge_tangents_swap_on_reverse() {
    let mut edge = Edge3d::with_tangents(
...
fn edge_ray_intersection_requires_forward_progress() {
    let edge = Edge2d::new(Vector2d::new(0.0, 0.0), Vector2d::new(5.0, 0.0));
    let other = Edge2d::new(Vector2d::new(-1.0, -1.0), Vector2d::new(-1.0, 1.0));
    assert!(edge.ray_intersection_with_edge(&other).is_none());

    let crossing = Edge2d::new(Vector2d::new(2.0, -1.0), Vector2d::new(2.0, 1.0));
